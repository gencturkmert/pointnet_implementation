...
class ModelNetDataset(Dataset):
    def __init__(
        ...
        if dataset == "ModelNet40":
            ...
            raise ValueError(
                "Invalid dataset. Choose either 'ModelNet10' or 'ModelNet40'."
            )
        ...
        categories = [
            folder
            for folder in os.listdir(os.path.join(self.root_dir, self.dataset))
            if os.path.isdir(os.path.join(self.root_dir, "ModelNet10", folder))
        ]
        ...
        for category in categories:
            ...
            temp_filepaths.extend(
                [(file, categories.index(category)) for file in files]
            )  # Store file paths and their corresponding labels

        if split == "train":
            self.filepaths, _ = train_test_split(
                temp_filepaths,
            ...
            _, self.filepaths = train_test_split(
                temp_filepaths,
                train_size=split_ratio,
                test_size=1 - split_ratio,
                random_state=42,
    ...
    def load_mesh(filepath):
        ...
        with open(filepath, "r") as file:
            ...
            if header.startswith("OFF"):
                if header == "OFF":
                    # The counts are on the second line
                    parts = lines[1].strip().split()
                    vertex_start_index = 2
                else:
                    # The counts are on the same line just after 'OFF'
            ...
            # Read vertices assuming each line has exactly three coordinates (x, y, z)
            for i in range(vertex_start_index, vertex_start_index + num_vertices):
